...
type DocumentCallback<T extends keyof DocumentEventMap> = (e: DocumentEventMap[T]) => void;
type WindowCallback<T extends keyof WindowEventMap> = (e: WindowEventMap[T]) => void;

export function useDocumentEvent<T extends keyof DocumentEventMap>(type: T, callback: DocumentCallback<T>) {
...
export function useWindowEvent<T extends keyof WindowEventMap>(type: T, callback: WindowCallback<T>) {
  const handler = useRef(callback);
  handler.current = callback;
